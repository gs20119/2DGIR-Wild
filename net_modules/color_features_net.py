...
class Color_net(nn.Module):
    ...
    def forward(self, inp, inf, inpf, view_direction=None,
        inter_weight=1.0,store_cache=False):
        oinp=inp
        if self.use_drop_out: inpf = self.drop_outs[0](inpf)
        if self.use_pencoding[0]:
            if self.use_decode_with_pos: oinp=inp.clone()
        ...
        if store_cache: self.cache_outd=outd
        else: self.cache_outd=None
            
        if self.use_decode_with_pos:
            outc=self.color_decoder(torch.cat([outd,oinp],dim=1))
        else:
            outc=self.color_decoder(torch.cat([outd,view_direction],dim=1)) 
        return outc
